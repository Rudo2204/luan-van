...
Để chương trình không xảy ra undefined behavior thì trong ngôn ngữ C/C++ người lập trình phải nắm rõ các edge cases có thể xảy ra undefined behavior, từ đó mà viết mã nguồn để tránh các trường hợp này.
Điều này là một gánh nặng tương đối lớn mà người lập trình phải nắm và hiểu rõ để có thể thực hiện phát triển các hệ thống có độ tin cậy cao.
...
Type safety (an toàn kiểu) là một trong những thuộc tính mà một chương trình well defined luôn có \cite{phd_safe_type}.
...
\end{longtable}
Như đã phân tích trên, mặc dù các khối unsafe là một công cụ mạnh, cho phép người lập trình có thêm một công cụ mạnh mẽ và tạm thời bỏ qua sự kiểm tra nghiêm ngặt của bộ compiler của Rust để thực hiện được những thao tác lập trình nâng cao hơn.
...
Và cộng thêm việc sử dụng \emph{rust-gdb} cùng với tính năng pretty-printing làm cho việc debug một chương trình Rust không gặp quá nhiều khó khăn, thậm chí là có phần suôn sẻ nếu người dùng có nhiều kinh nghiệm sử dụng bộ công cụ này.
...
\subsection{Bậc hỗ trợ chính thức của Rust}
...
Các đối tượng này được nhà phát triển ngôn ngữ Rust cung cấp sẵn cho binary của các tool trong Cargo, được hỗ trợ biên dịch, testing tự động và đầy đủ trên các dịch vụ CI/CD và là đối tượng có tài liệu đầy đủ và hoàn chỉnh nhất.
Các tài liệu này được viết và duy trì bởi các thành viên chính thức của của ngôn ngữ này.
